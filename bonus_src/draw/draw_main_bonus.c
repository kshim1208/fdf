...
/*   Updated: 2022/11/03 12:11:30 by kshim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_fdf_univ *univ, int row, int col)
{
	...
	if (univ -> rend_val -> ortho_persp == 1)
	{
		if (ft_check_vertex_w_for_clipping(
				&(univ -> world_guide[row][col]), univ) == 0)
			return ;
	}
	if (row != 0)
		ft_draw_line(
