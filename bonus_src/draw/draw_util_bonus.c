...
/*   Updated: 2022/11/03 12:11:23 by kshim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_vertex_w_for_clipping(t_fdf_pnt *now_pnt, t_fdf_univ *univ)
{
	if (now_pnt -> vertex_tfmed[3] <= 0
		|| now_pnt -> vertex_tfmed[0] < -1 * now_pnt -> vertex_tfmed[3]
		|| now_pnt -> vertex_tfmed[0] > now_pnt -> vertex_tfmed[3]
		|| now_pnt -> vertex_tfmed[1] < -1 * now_pnt -> vertex_tfmed[3]
		|| now_pnt -> vertex_tfmed[1] > now_pnt -> vertex_tfmed[3]
		|| now_pnt -> vertex_tfmed[2] < -1 * now_pnt -> vertex_tfmed[3]
		|| now_pnt -> vertex_tfmed[2] > now_pnt -> vertex_tfmed[3])
	{
		now_pnt -> vertex_tfmed[3] = 0;
		return (0);
	}
	now_pnt -> vertex_tfmed[0] = now_pnt -> vertex_tfmed[0]
		/ now_pnt -> vertex_tfmed[3];
	now_pnt -> vertex_tfmed[1] = now_pnt -> vertex_tfmed[1]
		/ now_pnt -> vertex_tfmed[3];
	now_pnt -> vertex_tfmed[2] = now_pnt -> vertex_tfmed[2]
		/ now_pnt -> vertex_tfmed[3];
	now_pnt -> vertex_tfmed[3] = 1;
	now_pnt -> color = now_pnt -> color;
	ft_multiply_matrix_44_14(univ -> tf_all_matrices -> viewpt_tf_all,
		now_pnt -> vertex_tfmed, univ -> matrix_buf -> matrix_buf_1x4);
	ft_matrix_copy_14(univ -> matrix_buf -> matrix_buf_1x4,
		now_pnt -> vertex_tfmed);
	return (1);
}
