...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   set_transform_matrix2_bonus.c                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: kshim <kshim@student.42seoul.kr>           +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/31 16:03:04 by kshim             #+#    #+#             */
/*   Updated: 2022/11/03 12:09:03 by kshim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_set_pers_projection_transform_matrix(t_fdf_rend_val *rend_val,
			t_project_tf_data *prj_tf_data, float prj_tf_all[4][4])
{
	float	tmp_cvv[4];

	ft_set_rtnlbf_from_cvv(
		rend_val, prj_tf_data, tmp_cvv, prj_tf_data -> pers_scale);
	prj_tf_all[0][0] = (2 * prj_tf_data -> rtn[2])
		/ (prj_tf_data -> rtn[0] - prj_tf_data -> lbf[0]);
	prj_tf_all[0][2] = (prj_tf_data -> rtn[0] + prj_tf_data -> lbf[0])
		/ (prj_tf_data -> rtn[0] - prj_tf_data -> lbf[0]);
	prj_tf_all[1][1] = (2 * prj_tf_data -> rtn[2])
		/ (prj_tf_data -> rtn[1] - prj_tf_data -> lbf[1]);
	prj_tf_all[1][2] = (prj_tf_data -> rtn[1] + prj_tf_data -> lbf[1])
		/ (prj_tf_data -> rtn[1] - prj_tf_data -> lbf[1]);
	prj_tf_all[2][2] = -1 * ((prj_tf_data -> lbf[2] + prj_tf_data -> rtn[2])
			/ (prj_tf_data -> lbf[2] - prj_tf_data -> rtn[2]));
	prj_tf_all[0][3] = 0;
	prj_tf_all[1][3] = 0;
	prj_tf_all[2][3] = -1 * (
			(2 * (prj_tf_data -> lbf[2] * prj_tf_data -> rtn[2]))
			/ (prj_tf_data -> lbf[2] - prj_tf_data -> rtn[2]));
	prj_tf_all[3][2] = -1;
	prj_tf_all[3][3] = 0;
	return ;
